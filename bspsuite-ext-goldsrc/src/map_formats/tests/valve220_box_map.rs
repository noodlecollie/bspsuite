...
use bspextifc::builders::map_source_builder::{
	Brush, BrushFace, BuilderError, BuilderResult, Entity, MapSourceBuilder,
	MapSourceBuilderApiProvider,
};
...
fn parse_box_map()
{
	...
	let result = parse_map(
		BOX_MAP_SOURCE,
		&mut MapSourceBuilderApiProvider::new(MapSourceBuilder::new(build_result.borrow_mut())),
	);
