...
pub fn new(normal: Vec3, dist: Float) This {
    ...
    std.debug.assert(std.math.approxEqRel(Float, normal.length2(), 1.0, math.constants.zero_epsilon));
...
pub fn newFromDir(dir: Vec3, dist: Float) This {
    ...
    return if (math.utils.vec3ApproxZero(normal)) This.null_plane else new(normal, dist);
}
...
pub fn intersectionPointWithLine(this: This, p0: Vec3, p1: Vec3) ?Vec3 {
    ...
    if (math.utils.floatApproxZero(normal_dot_line_dir)) {
...
pub fn basisVectors(this: This) [2]Vec3 {
    return switch (this.normal) {
        ...
        .nonaxial => |norm_vec| {
            if (math.utils.vec3ApproxZero(norm_vec)) {
            ...
            std.debug.assert(!math.utils.vec3ApproxZero(first_basis));
...
test "Points can be projected onto a plane via its normal" {
    ...
    try testing.expect(expected_projected_point.approxEqRel(actual_projected_point, math.constants.zero_epsilon));
}
...
test "The intersection point can be determined between the plane and a line" {
    ...
    try testing.expect(expected_intersection_point.approxEqRel(actual_intersection_point, math.constants.zero_epsilon));
...
test "Basis vectors can be generated from a plane" {
...

    {
        ...
        try testing.expect(basis_vectors[0].approxEqRel(expected_basis_right, math.constants.zero_epsilon));
        try testing.expect(basis_vectors[1].approxEqRel(expected_basis_up, math.constants.zero_epsilon));
    }
